...
refine 阶段在整理第 K 段时，识别当前段引用的前文概念（前 K-1 段中已经讲过的术语 / 主题），记录到 `RefinedSegment.cross_refs`（前置段的 id 列表），并允许在 `cleaned_text` 中嵌入 `[§N]` 形式的内嵌引用。合并阶段消费这些字段把笔记渲染为带章节内交叉引用的 Markdown。

**本管线只产出引用关系，不渲染链接**——渲染是 `merge/` 的职责。
...
- 元信息序列化为 JSON 一并落盘，下游读元信息不需要再 probe 一次
...
**Output**：`Transcript`（schema 见 §4）。落盘 `cache/{hash}/transcript_raw.json`。
...
**Output**：`SegmentList`（schema 见 §4）。落盘 `cache/{hash}/segments.json`。
...
- 输出 JSON 的解析与校验：
  1. 解析失败 → 重试 1 次（让 LLM "请只输出 JSON 数组，不要其他文字"）；仍失败抛 `LLMError`，**不要无限重试浪费 token**
  2. 校验时间戳：`start < end`、`markers[i].end == markers[i+1].start`（首尾相邻）、`markers[0].start == 0`、`markers[-1].end == transcript.duration`（容差 ±200ms）
  3. 校验段时长：每段 `end - start` 在 `[min_segment_seconds, max_segment_seconds]` 内
  4. 任一校验失败抛 `LLMError` 含具体不变量名，触发上层重试
...
**缓存键**：`hash(Transcript) + hash(segment 配置) + hash(LLM profile) + hash(prompts/segment.jinja) + "segment"`。

**错误处理**：
- LLM JSON 解析失败：见上，1 次重试后上抛 `LLMError`
- LLM 输出违反不变量：上抛 `LLMError`，不自动"修复"（自动修复会掩盖 prompt 缺陷）
- LLM endpoint 5xx / 限流：`tenacity` 装饰器自动重试，重试耗尽转 `TransportError` / `RateLimitError`
...
**Output**：每段 `RefinedSegment` 落盘到 `cache/{hash}/refined/{seg_id:04d}.json`，全部完成后汇总为 `RefinedTranscript`，落盘 `cache/{hash}/refined_transcript.json`。
...
*Sliding window*：当 system message token 数超过 `sliding_window_token_threshold` 时，进入 sliding window 模式：
...
进入 sliding window 模式会让 prompt cache 失效（前缀变了）。这是 token 成本与 cache 命中之间的权衡，由 `sliding_window_token_threshold` 控制阈值。

*跨段术语呼应*（见 §2.4）：prompt 中明确指示 LLM "如果当前段引用了之前段的概念，把对应 segment id 列入 `cross_refs`，并可在 `cleaned_text` 中使用 `[§N]` 形式标注内嵌引用"。

LLM 输出的 `cross_refs` 必须满足 `all(ref < current_id for ref in cross_refs)`（只能引用之前的段）。校验失败触发该段重试。

*review_first*：调试开关。第一段 refine 后暂停，把产物打印给用户审核 + 可手工编辑 → 重新加载到累积文本。第一版默认关，留作开发期调风格用。

*断点续跑*：每完成一段就落盘 `refined/{seg_id:04d}.json`。stage 启动时扫描该目录，从未完成的最早一段继续。**这是 stage 内部的断点续跑，不是缓存机制**——stage 级缓存（见下"缓存键"）一旦失效（如 prompt 模板改了），所有 `refined/*.json` 一并清空重跑。
...
**缓存键**：stage 级，`hash(Transcript) + hash(SegmentList) + hash(refine 配置) + hash(LLM profile) + hash(prompts/refine.jinja) + "refine"`。命中时直接返回 `RefinedTranscript`，不进入 stage。未命中时进入 stage，扫描 `refined/*.json` 走断点续跑。

**错误处理**：
- 单段 LLM 失败：`tenacity` 重试，耗尽抛 `LLMError`，整个 refine 中止（已完成段保留落盘，下次重跑接续）
- LLM 输出 `cross_refs` 含未来段 id 或不存在的 id：抛 `LLMError`，触发该段重试（提示 LLM 编造段号）
- LLM 输出 JSON 解析失败：1 次内修重试，仍失败抛 `LLMError`

---

## 4. Schema

本管线相关的所有 dataclass 集中定义在 `core/schemas.py`。任何模块**只能引用、不能在自己模块内重新定义副本**（`coding_standards.md` §2.2）。
...
class SegmentList:
    ...
    target_count_hint: str          # 产物绑定生成时的 hint，便于回溯
...
class RefinedSegment:
    ...
    cleaned_text: str               # 去口水词、补标点、合并断句的整理结果，可含 [§N] 内嵌引用
...
class RefinedTranscript:
    ...
    language: str
    duration: float
    config_hash: str                # 产物绑定的配置 hash，用于缓存验证
```
...
5. `RefinedSegment.cross_refs` 中每个值都 `< self.id`，且对应到存在的 `RefinedSegment.id`
...
class AudioArtifacts:
    ...
    """音频管线是否已经全部跑完（refined_transcript.json 存在且校验通过）。
...
| 来源 | 允许？ |
|---|---|
| `core/`（schemas / paths / timestamps / pipeline / cache / config / context / logging / exceptions / artifacts / constants） | ✅ |
| `llm/` | ✅（仅 stage 3、4） |
| `asr/` | ✅（仅 stage 2） |
| `media/` | ✅（仅 stage 1） |
| `visual_pipeline/` | ❌ |
...
1. 主路径用真实输入跑通端到端（`tests/fixtures/` 下的 30 秒音频片段）
2. 缓存机制工作（再跑一次能命中缓存，跳过该 stage 的实际计算）
3. 错误路径有测试覆盖（至少 1 个错误输入 → 抛预期异常的测试）
4. 类型检查通过（`pyright` 或 `mypy --strict`）
