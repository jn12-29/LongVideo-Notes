...
项目根目录配置 `pyright` 或 `mypy --strict`。CI 阶段类型检查不通过则 PR 不能合并。
...
所有跨模块共享的 dataclass 集中在 `core/schemas.py`。其他模块**只能引用，不能在自己模块内重新定义副本**。
...
- 日志条目应可被 grep：包含可识别的 stage 名 / 阶段名 / video_hash
...
- 30 秒音频片段（`.wav`、`.mp3` 各一份）
- 30 秒视频片段（含 PPT 切换、含板书）
- 几个真实但短的转录 JSON
- 几张真实关键帧图片
...
- 我在定义跨模块的数据类型吗？→ 加到 `core/schemas.py`，不要在本模块定义
...
1. 写代码前先读 `core/schemas.py` 看有什么类型可用
2. 读至少一个已实现的同类模块作为风格参照（README 第 7 节会指明参照对象）
...
`pyproject.toml` 中的版本约束是**经过验证的**。不要：

- "升级到最新版"
- "降级到我熟悉的版本"
- 引入文档没列出的新依赖

如果你认为需要新依赖，先在 PR 中提出讨论，不要直接 `pip install` + 加进 `pyproject.toml`。
...
- 测试用真实小输入文件（`tests/fixtures/`），不是凭空构造的"看起来像"的字符串
...
```python
# 按 docs/audio_pipeline.md §4.3，refine 阶段的 prompt cache 优化要求
...
不要试图一次性把整个管线写完。按 README 第 7 节的"实现优先级"顺序：
...
@dataclass
class Foo: ...  # 不在 core/schemas.py 但需要跨模块用
